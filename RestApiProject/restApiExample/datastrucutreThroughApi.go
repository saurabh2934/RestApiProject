...
func NormalSearch(req *restful.Request, res *restful.Response) {
	...
	s := RestApiProject.LinearSearching(ar.Arr, find) // calling function of linear search to search element
	res.WriteAsJson(s)
}

// http://localhost:8080/DataStrucure/Searching/binarySearch
func BinarySearch(req *restful.Request, res *restful.Response) {
	...
	fmt.Println(ar)
	find := ar.F // initializing find variable which to be search
	s := RestApiProject.BinarySearch(ar.Arr, find)
	res.WriteEntity(s)
}
